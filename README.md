...
buat file install.sh lalu paste skrip dibawah simpan tekan ctrl+x y enter
```sh
nano install.sh
```
```sh
...
log_step() { echo -e "${YELLOW}==> $1...${RESET}"; }
log_success() { echo -e "${GREEN}✔ $1 selesai${RESET}"; }
log_fail() { echo -e "${RED}✘ $1 gagal${RESET}"; exit 1; }
...
log_success "Update & upgrade"

# Cek versi Python
PY_VER=$(python3 -c "import sys; print(f'{sys.version_info.major}.{sys.version_info.minor}')")
if [[ "$PY_VER" < "3.12" ]]; then
  echo -e "${YELLOW}⚠ Python versi $PY_VER terdeteksi, sebaiknya >=3.12${RESET}"
fi
...
retry() {
    ...
    if [ $n -eq $max ]; then
        log_fail "Gagal menjalankan: $*"
    fi
}

# Install Node.js & Yarn dengan retry
log_step "Menginstal Node.js 22"
retry bash -c "curl -fsSL https://deb.nodesource.com/setup_22.x | $SUDO bash -"
...
yarn -v
log_success "Node.js & Yarn"

# Clone rl-swarm dengan deteksi folder
log_step "Mengkloning repository rl-swarm"
if [ -d "rl-swarm" ]; then
    echo "Folder rl-swarm sudah ada, masuk ke folder."
else
...
# Deteksi driver & CUDA
log_step "Mengecek NVIDIA driver & CUDA"
if command -v nvidia-smi &>/dev/null; then
    nvidia-smi
    DRIVER=$(nvidia-smi --query-gpu=driver_version --format=csv,noheader | head -1)
    CUDA=$(nvidia-smi --query-gpu=cuda_version --format=csv,noheader | head -1)
    echo -e "${GREEN}✔ Driver NVIDIA terdeteksi: $DRIVER, CUDA: $CUDA${RESET}"
else
    echo -e "${RED}❌ NVIDIA driver tidak terdeteksi.${RESET}"
fi

echo -e "\n✅ Virtual environment aktif."
echo -e "Untuk menginstal PyTorch GPU sesuai driver/CUDA Anda, klik link berikut di dalam venv:"
